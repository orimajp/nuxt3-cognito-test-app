...
//    const publicKyes = await $fetch<PublicKeys>(url)
    const publicKyes:AxiosResponse<PublicKeys> = await axios(option)
//    cacheKeys = publicKyes.keys.reduce((agg, current) => {
    cacheKeys = publicKyes.data.keys.reduce((agg, current) => {
...
export default defineEventHandler(async (event) => {
  if (
    ...
    event.req.url.startsWith('/api/token')
  ) {
    return
  }
...
//    console.log(event.req.headers)
    console.log(event.req.headers.authorization) // Client Side
    console.log(event.req.headers.Authorization) // Server Side
    const authorization = event.req.headers?.authorization ? event.req.headers.authorization
      : event.req.headers?.Authorization as string
...
//    console.log('token={}', token)
    ...
    if (tokenSections.length < 2) {
      ...
      event.res.statusCode = 401
      event.res.setHeader('WWW-Authenticate', 'Bearer error="invalid_token1"')
      event.res.end('Unauthorized')
      return
    }
    ...
    if (key === undefined) {
      ...
      event.res.statusCode = 401
      event.res.setHeader('WWW-Authenticate', 'Bearer error="invalid_token2"')
      event.res.end('Unauthorized')
      return
    }
...
//    console.log('calim=', claim)
    ...
    if (currentSeconds > claim.exp || currentSeconds < claim.auth_time) {
      ...
      event.res.statusCode = 401
      event.res.setHeader('WWW-Authenticate', 'Bearer error="invalid_token3"')
      event.res.end('Unauthorized')
      return
    }
    if (claim.iss !== cognitoIssuer) {
      ...
      event.res.statusCode = 401
      event.res.setHeader('WWW-Authenticate', 'Bearer error="invalid_token4"')
      event.res.end('Unauthorized')
      return
    }
    if (claim.token_use !== 'id') {
      ...
      event.res.statusCode = 401
      event.res.setHeader('WWW-Authenticate', 'Bearer error="invalid_token5"')
      event.res.end('Unauthorized')
      return
    }
//    event.context.auth = claim
    ...
    console.log(error)
    event.res.statusCode = 401
    event.res.setHeader('WWW-Authenticate', 'Bearer error="invalid_token6"')
//    event.res.end('Unauthorized')
    event.res.end(error)
  }
})
